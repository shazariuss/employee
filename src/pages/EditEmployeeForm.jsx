...
const EditEmployeeForm = () => {
    ...
    const fetchEmployeeData = async () => {
        try {
            ...
            setFormData({
                ...
                employment_date: new Date(employeeData.employment_date)
                    .toISOString()
                    .split("T")[0],
                ...
                emergency_contact_name:
                    employeeData.emergency_contact.contact_name,
                emergency_contact_relationship:
                    employeeData.emergency_contact.relationship,
                emergency_contact_number:
                    employeeData.emergency_contact.phone_number,
            });
        } catch (error) {
    ...
    const handleChange = (e) => {
        ...
        if (name === "full_name") {
            ...
            if (value.length !== 9) {
                newErrors[name] =
                    "Номер паспорта должен состоять из 9 символов.";
            } else {
                delete newErrors[name];
            }
            ...
            if (value.length < 12) {
                newErrors[name] =
                    "Номер телефона должен содержать не менее 12 цифр и без +.";
            } else {
                delete newErrors[name];
            }
        } else if (name === "email") {
            ...
            if (!value || employmentDate > new Date()) {
                newErrors[name] =
                    "Дата приема на работу должна быть в прошлом.";
            } else {
                delete newErrors[name];
            }
        } else if (name === "degree") {
            ...
            if (value.length < 12) {
                newErrors[name] =
                    "Номер телефона контактного лица должен содержать не менее 12 цифр и без +.";
            } else {
                delete newErrors[name];
            }
        } else {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    await messageApi.open({
                        type: "success",
                        content:
                            "Произошла ошибка при обновлении данных сотрудника",
                    });
                }
            } catch (error) {
        ...
        <>
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block font-medium">
                            Имя контактного лица в экстренной ситуации(ФИО):
                        </label>
                        <input
